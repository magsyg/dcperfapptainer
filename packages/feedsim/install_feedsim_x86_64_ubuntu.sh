...
if ! [ -d "cmake-3.14.5" ]; then
    ...
    tar -zxf "cmake-3.14.5.tar.gz" --no-same-owner
    cd "cmake-3.14.5"
    mkdir staging
...
if ! [ -d "gengetopt-2.23" ]; then
    ...
    tar -xf "gengetopt-2.23.tar.xz" --no-same-owner
    cd "gengetopt-2.23"
    ./configure
    make -j"$(nproc)"
    make install
...
if ! [ -d "boost_1_71_0" ]; then
    ...
    tar -xzf "boost_1_71_0.tar.gz" --no-same-owner
    cd "boost_1_71_0"
...
if ! [ -d "gflags-2.2.2" ]; then
    ...
    tar -xzf "gflags-2.2.2.tar.gz" --no-same-owner
    cd "gflags-2.2.2"
...
if ! [ -d "glog-0.4.0" ]; then
    ...
    tar -xzf "glog-0.4.0.tar.gz" --no-same-owner
    cd "glog-0.4.0"
    mkdir -p build && cd build
    cmake -DBUILD_SHARED_LIBS=ON -DBUILD_TESTING=OFF -DCMAKE_BUILD_TYPE=Release ../
    make -j8
...
if ! [ -d "jemalloc-5.2.1" ]; then
    ...
    echo "hi2 \n"
    tar -xvf "jemalloc-5.2.1.tar" --no-same-owner
    echo "hi3 \n"
    cd "jemalloc-5.2.1"
...
if ! [ -d "libevent-2.1.11-stable" ]; then
    ...
    tar -xzf "libevent-2.1.11-stable.tar.gz" --no-same-owner
